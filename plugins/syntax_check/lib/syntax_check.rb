      ...
      Redcar::Menu::Builder.build do
        sub_menu "Plugins" do
          lazy_sub_menu "Syntax Checking" do
            item "Enabled", :type => :check, :checked => proc { SyntaxCheck.enabled? }, :command => ToggleSyntaxChecking
            separator
            Checker.checkers.each do |checker|
              grammar = checker[0]
              item "#{grammar}", :type => :check, :checked => proc { SyntaxCheck.check_type?(grammar) }, :command => ToggleGrammarChecking, :value => grammar
            end
          end
        end
      end
